...
function EmptyState({ onCreated }: { onCreated?: () => void }) {
  return (
    <div className="flex flex-col items-center justify-center py-16 text-center">
      ...
      <CreateVoyageDialog onCreated={onCreated} />
    </div>
  );
}
...
export default function VoyagesPage() {
  ...
  useEffect(() => {
    ...
    async function fetchVoyages() {
      let query = supabase
        .from("voyages")
        .select(`id, voyage_reference, vessel_id, voyage_number, created_at, vessels(name), owner_names(name), charterer_names(name), cargo_names(name)`)
    ...
    fetchVoyages();
  }, [tenantId, page, search, refreshKey]);
      ...
      <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-4 mb-6">
        <div className="flex items-center gap-3">
          ...
          <div className="relative flex-1">
            ...
            <CreateVoyageDialog onCreated={() => setRefreshKey(k => k + 1)} />
        </div>
      </div>
            ...
            {loading ? (
              <TableRow>
                <TableCell colSpan={7} className="text-center py-12">
                  <span className="text-ocean-400 animate-pulse">Loading voyages...</span>
                </TableCell>
              </TableRow>
            ) : voyages.length === 0 ? (
              <TableRow>
                  <TableCell colSpan={7} className="p-0">
                  <EmptyState onCreated={() => setRefreshKey(k => k + 1)} />
                </TableCell>
              </TableRow>
            ) : (
              voyages.map((voyage) => (
                <TableRow key={voyage.id} className="group hover:bg-ocean-50/60 transition">
                  ...
                  <TableCell>{voyage.owner_names?.name || <span className="text-ocean-300">—</span>}</TableCell>
                  <TableCell>{voyage.charterer_names?.name || <span className="text-ocean-300">—</span>}</TableCell>
